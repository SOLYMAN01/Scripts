Config = {}
Config.SeeOwnLabel = true
Config.SeeDistance = 100
Config.TextSize = 1.2
Config.ZOffset = 1.2
Config.NearCheckWait = 500
Config.TagByPermission = false --Using xPlayer.getPermissions() which is deprecated method for old ESX
Config.GroupLabels = {
    helper = "SOPORTE",
    mod = "~g~MODERADOR",
    admin = "~b~STAFF",
    superadmin = "~r~SUPER ADMIN",
}

Config.PermissionLabels = {
    [1] = "HELPER",
    [2] = "~g~MODERATOR",
    [3] = "~b~ADMINISTRATOR",
    [4] = "<img src='https://img.icons8.com/color/17/000000/moderator-male--v1.png'/>",
    [5] = "<img src='https://img.icons8.com/color/17/000000/moderator-male--v1.png'/>",
}
